...
#     VAD = json.load(open(config.data_dir+"/VAD.json",
#                     "r", encoding="utf-8"))  # NRC_VAD
#     CN = csv.reader(open(config.data_dir+"/assertions.csv",
#                     "r", encoding="utf-8"))  # ConceptNet raw file

#     concept_dict = {}
#     concept_file = open(config.data_dir+"/ConceptNet.json",
#                         "w", encoding="utf-8")
...
#                 c1_c2_sim = torch.cosine_similarity(
#                     c1_vector, c2_vector, dim=0).item()
...
#                         concept_dict[c1][c2] = [relation, c2_vad,
#                                                 c1_c2_sim, weight, emotion_gap, score]
#                     else:
#                         concept_dict[c1] = {}
#                         concept_dict[c1][c2] = [relation, c2_vad,
#                                                 c1_c2_sim, weight, emotion_gap, score]
...
#                         concept_dict[c2][c1] = [relation, c1_vad,
#                                                 c1_c2_sim, weight, emotion_gap, score]
#                     else:
#                         concept_dict[c2] = {}
#                         concept_dict[c2][c1] = [relation, c1_vad,
...
#     concept_dict = json.load(
#         open(config.data_dir+"/ConceptNet.json", "r", encoding="utf-8"))
#     rank_concept_file = open(
#         config.data_dir+'/ConceptNet_VAD_dict.json', 'w', encoding='utf-8')

#     rank_concept = {}
#     for i in concept_dict:
#         # [relation, c1_vad, c1_c2_sim, weight, emotion_gap, score]   relation, weight, score
#         rank_concept[i] = dict(sorted(concept_dict[i].items(
#         ), key=lambda x: x[1][5], reverse=True))  # 根据vad由大到小排序
#         rank_concept[i] = [[l, concept_dict[i][l][0], concept_dict[i][l][1], concept_dict[i][l][2],
#                             concept_dict[i][l][3], concept_dict[i][l][4], concept_dict[i][l][5]] for l in concept_dict[i]]

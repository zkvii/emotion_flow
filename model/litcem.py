...
from model.translator.cem_translator import Translator
...
class CEM(LightningModule):
    ...
    def train_one_batch(self, batch,batch_idx,train=True):
        (
            enc_batch,
            _,
            _,
        ...
        src_mask, ctx_output, emo_logits = self(batch)
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        ys = torch.ones(1, 1).fill_(config.SOS_idx).long().to(src_mask.device)
        ...
        for i in range(max_dec_step + 1):
            ...
            ys = torch.cat(
                [ys, torch.ones(1, 1).long().fill_(next_word).to(src_mask.device)],
                dim=1,
            )
            mask_trg = ys.data.eq(config.PAD_idx).unsqueeze(1)

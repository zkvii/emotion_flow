...
class EMF(LightningModule):
    ...
    def forward(self, input_batch, target_batch, target_program):
        # divide data
        # dec_batch = target_batch[:, 1:]
        sos_token = (
            torch.LongTensor([config.SOS_idx] * target_batch.size(0)).unsqueeze(1)
        ).to(self.device)
        dec_batch_shift = torch.cat((sos_token, target_batch[:, :-1]), 1)
        ref_batch = target_batch
        src_mask = self.make_src_mask(input_batch)
        trg_mask = self.make_trg_mask(dec_batch_shift)
        # encode
        enc_output,*_= self.encoder(input_batch, src_mask)
        dec_output,*_= self.decoder(dec_batch_shift, trg_mask, enc_output, src_mask)
    ...
    def test_step(self, batch, batch_idx):
        ...
        decoded_words = self.decoder_greedy(input_batch, max_length=1000)
        
        
        outputs = open(file_path, 'a+', encoding='utf-8')

        rf = " ".join(batch["target_txt"][0])
        # outputs.write("Emotion:{} \n".format(batch["program_txt"][0]))
        outputs.write(f"Emotion:{batch['program_txt'][0]} \n")
        outputs.write(f'Context:{[" ".join(s) for s in batch["input_txt"][0]]} \n')
        outputs.write(f"Pred:{decoded_words} \n")
    ...
    def decoder_greedy(self, input_batch, max_length):
        ...
        for i in range(config.max_dec_step):
            ...
            decoded_words.append([
                    "<EOS>"
                    if ni == config.EOS_idx
                    else self.vocab.index2word[ni]
